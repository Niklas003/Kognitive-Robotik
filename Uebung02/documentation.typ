...
Wie bereits im Versuchsaufbau angedeutet wird mit einem Gyroskop die Winkelgeschwindigkeit um eine Achse (X, Y, Z) gemessen. Beispielswiese in °/s oder Rad/s
== Mögliche Szenarien
...
In dem Ergebnisdiagramm wird über die Zeit (Timesteps) hinweg die Distanz zu einem Objekt dargestellt. Im ersten Abschnitt des Diagrammes (bis Timestep 5000) fährt der Roboter auf eine über ihm schwebende Wand zu. Hierbei verringert sich die Distanz zum Objekt, welches auch vom Sonar erfasst wird. Ca. ab Timestep 35000 erfasst das Sonar wieder ein sich näherndes Objekt an welches bis auf Distanz = 0 herangefahren wird. Danach steigt die vom Sonar erfasste Distanz stark an, was jedoch an der Positionierung des Sonars liegt. Zu beachten ist, dass in Webots Sonarwellen welche aus weniger als 22,5° auf ein Objekt treffen keinen Distanzwert liefern, um den physikalischen Gesetzmäßigkeiten gerecht zu werden. 
== Physikalische Prinzipien
...
In den gezeigten Bildern ist die "Sicht" des Lidars im 3D-Raum zu erkennen. Wobei die blauen Punkte die maximale Reichweite des Lidar zeigen, ist durch die Roten Punkte in der Wand gekennzeichnet, in dessen Richtung der Thymio II sich dreht. Im Bild lässt sich somit nicht nur die Position des Objektes in Relation zum Roboter entnehmen, sondern auch seine Form.
== Physikalische Prinzipien
Ein Lidar funktioniert ähnlich wie ein Sonar oder Radar über das Aussenden eines Signals und das Auswerten der Reflektion des Signals. Wobei das Sonar und Radar, Schall und elekrtomagnetische Strahlung als Signal nutzen, nutzt das Lidar ausgestrahltes Licht. Das Lidar strahlt dabei einen oder mehrere konzentrierte Lichtstrahlen aus. Diese werden dann durch Objekte in der Umgebung reflektiert und in das Lidar zurückgestrahlt. Durch das Messen der vergangenen Zeit zwischen Ausstrahlung und Einfangen eines Strahls kann dann die Entfernung des Objektes ermittelt werden. Mit Hilfe weiteren Strahlen, deren Reflektion und Winkel, kann dann eine sogenannte Punktwolke gebildet werden, die z.B.: die Form eines Objektes widerspiegelt.  
== Mögliche Szenarien
Mögliche Einsatzgebiete sind die Umgebung und Objekterkennung. Optimalerweise in Kombination mit weiteren Sensoren, wie zum Beispiel einer Kamera mit Bilderkennung.
